...
public class XUtils {
    ...
    private static String x_DB_NAME = "db_menshen";
...
//        // 创建xUtils3数据库
     ...
     * @param idValue
     */
    public static <T> T findById(Class var1, String idValue) {
        T t = null;
        if (x_dbManager == null) {
            x_dbManager = oncreateUtils();
        }
        ...
        try {
            t = (T) x_dbManager.findById(var1, idValue);
            return t;
        } catch (DbException e) {
            e.printStackTrace();
        }
     ...
     * @param value
     */
    public static <T> List findBySelector(Class var1, String key, String value) {
        List<T> list = null;
        if (x_dbManager == null) {
            x_dbManager = oncreateUtils();
        }
        try {
            Selector selector = x_dbManager.selector(var1).where(key, "=", value);
            //如果要使用条件查询的话可以使用
